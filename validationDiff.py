...
def denoise_pred_origin(pred: np.ndarray):
    """
...
class AddOrganPseudo(Transform):
    def __call__(self, data):
        image_path = data['image']
        organ_pseudo_dir = "organ_pseudo_swin_new/liver"
        filename = os.path.basename(image_path)
        organ_pseudo = os.path.join(organ_pseudo_dir, filename)
...
def main():
    ...
    with torch.no_grad():
        for idx, val_data in enumerate(val_loader):
            ...
            val_outputs, val_labels, val_organ_pseudo = val_data[0]['pred'], val_data[0]['label'], val_data[0][
                'organ_pseudo']
            ...
            row = [name, current_liver_dice, current_liver_nsd, current_tumor_dice, current_tumor_nsd,
                   current_tumor_sensitivity, current_tumor_specificity]
            rows.append(row)

            print(name, val_outputs[0].shape,
                  'dice: [{:.3f}  {:.3f}]; nsd: [{:.3f}  {:.3f};]; sen: [{:.3f}]; spe: [{:.3f}]'.format(
                      current_liver_dice, current_tumor_dice, current_liver_nsd, current_tumor_nsd,
                      current_tumor_sensitivity, current_tumor_specificity),
        ...
        rows.append(['average', liver_dice, liver_nsd, tumor_dice, tumor_nsd, tumor_sensitivity, tumor_specificity])
